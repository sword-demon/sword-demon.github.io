---
title: Go主机安全面试：Linux 主机事件丢失与采集链路排障
date: 2026-08-22 17:41:41
categories:
- Interview
tags:
- go
...
- 每个阶段都维护计数：`read_total`、`decode_failed`、`queue_dropped`、`upload_failed`。
- 事件结构里带 `sequence`、`source`、`host_id`、`boot_id` 和采集时间。
...
- 主机重启、Agent 重启、进程崩溃后要结合 `boot_id`、`agent_start_id` 判断。
...
- `source` 可以是 `exec`、`file`、`network`、`audit` 等采集源。
...
- 诊断输出包含关联失败原因，例如 `process_cache_miss`、`pid_reused`、`namespace_mismatch`。
...
- 按 `host_id + source` 维护短窗口 reorder buffer。
...
- 支持按 `trace_id` 或 `sequence` 从 Agent 日志追到服务端日志。
...
- `CollectedAt` 和 `ReceivedAt` 分别用于攻击链排序和链路延迟分析。
...
- 掌握 Go 并发队列、worker 池、`context`、`atomic`、限流和有界缓存。
